  ...
  if(length(shiny.prefix) == 1){
    ...
    for (i in seq_along(shiny.prefix)) {
      ...
      if (file.exists(paste0(shiny.dir, "/", shiny.prefix[i], "image.rds"))) {
        ...
        readr::write_file(
          wrUImainS1(shiny.prefix[i], defPtSiz2[i], hasMultiSlides),
          file = fname,
          append = TRUE
        )
        readr::write_file(
          wrUImainS2(shiny.prefix[i], defPtSiz2[i], hasMultiSlides),
          file = fname,
          append = TRUE
        )
      }
      if (file.exists(paste0(shiny.dir, "/", shiny.prefix[i], "bw.rds"))) {
        readr::write_file(
          wrUImainT1(shiny.prefix[i]),
          file = fname,
          append = TRUE
        )
      }
      readr::write_file(
        wrUImainA1(shiny.prefix[i], defPtSiz[i]),
        file = fname,
        append = TRUE
      )
      readr::write_file(
        wrUImainS2(shiny.prefix[i], defPtSiz[1]),
        file = fname,
        append = TRUE
      )
      readr::write_file(
        wrUImainA2(shiny.prefix[i], defPtSiz[i]),
        file = fname,
        append = TRUE
      )
      readr::write_file(
        wrUImainA3(shiny.prefix[i], defPtSiz[i]),
        file = fname,
        append = TRUE
      )
      readr::write_file(
        wrUImainB1(shiny.prefix[i], defPtSiz[i]),
        file = fname,
        append = TRUE
      )
      readr::write_file(
        wrUImainB2(shiny.prefix[i]),
        file = fname,
        append = TRUE
      )
      readr::write_file(
